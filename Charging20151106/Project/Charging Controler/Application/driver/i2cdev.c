                ...
                uint16_t len, uint8_t *data)
{
  ...
  if (memAddress != I2CDEV_NO_MEM_ADDR)
  {
    ...
    status = I2C_Master_BufferWrite(I2Cx, buffer,  len + 1, DMA, devAddress << 1, I2C_TIMEOUT);
  }
  else
  {
    status = I2C_Master_BufferWrite(I2Cx, data,  len, DMA, devAddress << 1, I2C_TIMEOUT);
  }
...
{
  if(i2cdevWrite(I2C1, dev, reg,length, data))return 0;
  else return 1;
...
{
  if(i2cdevRead(I2C1, dev, reg,length, data))return 0;
  else return 1;
