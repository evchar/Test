...
#elif defined EMPL_TARGET_STM32L15X
#include "main.h"
#else
...
AKM_restore:
    ...
    st.chip_cfg.compass_addr = 0x1e;

    tmp[0] = 0x71;
    if (i2c_write(st.chip_cfg.compass_addr, 0x00, 1, tmp))
        goto AKM_restore;

    tmp[0] = 0xa0;
    if (i2c_write(st.chip_cfg.compass_addr, 0x01, 1, tmp))
        goto AKM_restore;

    tmp[0] = 0x00;//4
    if (i2c_write(st.chip_cfg.compass_addr, 0x02, 1, tmp))
        goto AKM_restore;

    do {
        delay_ms(10);
        if (i2c_read(st.chip_cfg.compass_addr, 0x09, 1, tmp))
            goto AKM_restore;
        if (tmp[0] & AKM_DATA_READY)
            break;
    } while (tries--);
    ...
    result = 0;
    data = (short)(tmp[0] << 8) | tmp[1];
    if ((data > 575) || (data < 243))
        result |= 0x01;
    data = (short)(tmp[2] << 8) | tmp[3];
    if ((data > 575) || (data < 243))
        result |= 0x02;
    data = (short)(tmp[4] << 8) | tmp[5];
    if ((data > 575) || (data < 243))
        result |= 0x04;
...
{
    unsigned char dadata[HWST_MAX_PACKET_LENGTH];
    unsigned char packet_count, ii;
    unsigned short fifo_count;
    ...
    dadata[0] = 0x01;
    dadata[1] = 0;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 2, dadata))
        return -1;
    delay_ms(200);
    dadata[0] = 0;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, dadata))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, dadata))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, dadata))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, dadata))
        return -1;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, dadata))
        return -1;
    dadata[0] = BIT_FIFO_RST | BIT_DMP_RST;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, dadata))
        return -1;
    delay_ms(15);
    dadata[0] = st.test->reg_lpf;
    if (i2c_write(st.hw->addr, st.reg->lpf, 1, dadata))
        return -1;
    dadata[0] = st.test->reg_rate_div;
    if (i2c_write(st.hw->addr, st.reg->rate_div, 1, dadata))
        return -1;
    if (hw_test)
        dadata[0] = st.test->reg_gyro_fsr | 0xE0;
    else
        dadata[0] = st.test->reg_gyro_fsr;
    if (i2c_write(st.hw->addr, st.reg->gyro_cfg, 1, dadata))
        return -1;

    if (hw_test)
        dadata[0] = st.test->reg_accel_fsr | 0xE0;
    else
        dadata[0] = test.reg_accel_fsr;
    if (i2c_write(st.hw->addr, st.reg->accel_cfg, 1, dadata))
        return -1;

    delay_ms(test.wait_ms);  //wait 200ms for sensors to stabilize

    /* Enable FIFO */
    dadata[0] = BIT_FIFO_EN;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, dadata))
        return -1;
    dadata[0] = INV_XYZ_GYRO | INV_XYZ_ACCEL;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, dadata))
        return -1;
    ...
    while (s < test.packet_thresh) {
		...
		if (i2c_read(st.hw->addr, st.reg->fifo_count_h, 2, dadata))
			return -1;
		fifo_count = (dadata[0] << 8) | dadata[1];
		packet_count = fifo_count / MAX_PACKET_LENGTH;
		...
		if (i2c_read(st.hw->addr, st.reg->fifo_r_w, read_size, dadata))
						return -1;
		ind = 0;
		for (ii = 0; ii < packet_count; ii++) {
			short accel_cur[3], gyro_cur[3];
			accel_cur[0] = ((short)dadata[ind + 0] << 8) | dadata[ind + 1];
			accel_cur[1] = ((short)dadata[ind + 2] << 8) | dadata[ind + 3];
			accel_cur[2] = ((short)dadata[ind + 4] << 8) | dadata[ind + 5];
			accel[0] += (long)accel_cur[0];
			accel[1] += (long)accel_cur[1];
			accel[2] += (long)accel_cur[2];
			gyro_cur[0] = (((short)dadata[ind + 6] << 8) | dadata[ind + 7]);
			gyro_cur[1] = (((short)dadata[ind + 8] << 8) | dadata[ind + 9]);
			gyro_cur[2] = (((short)dadata[ind + 10] << 8) | dadata[ind + 11]);
			gyro[0] += (long)gyro_cur[0];
			gyro[1] += (long)gyro_cur[1];
			gyro[2] += (long)gyro_cur[2];
...
//   /*add by hujiang*/
//   tmp = st.chip_cfg.int_enable | 0x20;//hujiang
//   if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &tmp))/*zero motion enable*/
//   return -1;//hujiang
//
//   if (i2c_write(st.hw->addr, 0x39, 1, &tmp))
//   return -1;//hujiang
//
//   tmp = 10; //Zero Motion Detection Threshold
//   if (i2c_write(0xd0, 0x21, 1, &tmp))
//   return -1;//hujiang
//
//   tmp = 1; //Zero Motion Detection Threshold
//   if (i2c_write(0xd0, 0x22, 1, &tmp))
//   return -1;//hujiang
...
static int setup_compass(void)
{
    ...
    data[0] = 0x8c;//BIT_I2C_READ | st.chip_cfg.compass_addr;//
    ...
    /* Enable slave 0, 9-byte reads. */
    data[0] = BIT_SLAVE_EN | 9;
    if (i2c_write(st.hw->addr, st.reg->s0_ctrl, 1, data))
        return -1;

    /* Slave 1 changes AKM measurement mode. */
    data[0] = 0x0c;//st.chip_cfg.compass_addr;//
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.chip_cfg.compass_addr, AKM_REG_ST1, 9, tmp))
        return -1;
    tmp[9] = AKM_SINGLE_MEASUREMENT;
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+9))
        return -1;
    mpu_set_bypass(0);
#endif
    ...
    if (i2c_write(st.chip_cfg.compass_addr, AKM_REG_CNTL, 1, tmp+8))
        return -1;
#else
    if (i2c_read(st.hw->addr, st.reg->raw_compass, 9, tmp))
        return -1;
#endif
#endif
    ...
    data[0] = ((long)data[0] * st.chip_cfg.mag_sens_adj[0]) >> 7;
    data[1] = ((long)data[1] * st.chip_cfg.mag_sens_adj[1]) >> 7;
    data[2] = ((long)data[2] * st.chip_cfg.mag_sens_adj[2]) >> 7;
...
int setup_compass(void)
{
    ...
    data[0] = 0;
    if (i2c_write(st.chip_cfg.compass_addr, 0x02, 1, data))
        return -1;


    data[0] = 0xf4;//不平均  //0xf0;//8次平均
    if (i2c_write(st.chip_cfg.compass_addr, 0x00, 1, data))
        return -1;


    data[0] = 0xa0;//4.7Ga
    if (i2c_write(st.chip_cfg.compass_addr, 0x01, 1, data))
        return -1;
    ...
    /* Slave 0 reads from AKM data registers. */
    data[0] = 0x9e;//BIT_I2C_READ | st.chip_cfg.compass_addr;//
    if (i2c_write(st.hw->addr, st.reg->s0_addr, 1, data))
        return -1;
    ...
    /* Slave 1 changes AKM measurement mode. */
    data[0] = 0x1e;//st.chip_cfg.compass_addr;//
    if (i2c_write(st.hw->addr, st.reg->s1_addr, 1, data))
        return -1;
...
int mpu_get_compass_reg(short *data, unsigned long *timestamp)
{
    ...
    if (i2c_read(st.chip_cfg.compass_addr, 0x03, 7, tmp))
        return -1;
    tmp[9] = AKM_SINGLE_MEASUREMENT;
    if (i2c_write(st.chip_cfg.compass_addr, 0x02, 1, tmp+9))
        return -1;
    mpu_set_bypass(0);
#endif
